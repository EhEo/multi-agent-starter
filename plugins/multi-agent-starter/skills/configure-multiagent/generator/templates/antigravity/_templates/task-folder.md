...
모든 worker 호출은 `_shared/backends.json` 정본 + 디스패처 `python3 _shared/adapters/call_worker.py <role> <brief-file>` → JSON envelope. Orchestrator가 envelope의 stdout을 `result.md`에 기록. 호출 전 승인.
